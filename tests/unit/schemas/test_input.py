...
class TestTenureValidation:
    ...
    def test_tenure_valid_values(self, valid_input_data, valid_tenure, total_for_tenure):
...
class TestMonthlyChargesValidation:
    ...
    @pytest.mark.parametrize("valid_monthly, total_for_monthly", [
        (0.01,   0.24),     # 0.01 × 24
        (50.0,   1200.0),   # 50.0 × 24
        (99.99,  2399.76),  # 99.99 × 24
        (119.0,  2856.0),   # 119.0 × 24
    ])
    def test_monthly_charges_valid_values(self, valid_input_data, valid_monthly, total_for_monthly):
    ...
    @pytest.mark.parametrize("invalid_value", [0.0, -1.0, 119.01, 200.0, "cinquenta", "abc", [1, 2], None])
...
class TestTotalChargesValidation:
    ...
    def test_total_charges_valid_values(self, valid_input_data, tenure, monthly, valid_total):
...
# ════════════════════════════════════════════════════════════════════════════════
@pytest.mark.parametrize("binary_field", [
    "senior_citizen",
    "partner",
    "dependents",
    "phone_service",
...
class TestBinaryFieldsValidation:
    ...
    def test_senior_citizen_valid_values(self, valid_input_data, binary_field, valid_binary):
...
class TestContractValidation:
    ...
    @pytest.mark.parametrize("valid_contract", [
        "Month-to-month",
        "One year",
        "Two year"
    ])
...
class TestPaymentMethodValidation:
    ...
    @pytest.mark.parametrize("valid_payment", [
        "Electronic check",
        "Mailed check",
...
class TestInternetServiceValidation:
    ...
    @pytest.mark.parametrize("valid_internet", [
        "Fiber optic",
        "DSL",
        "No"
    ])
...
class TestCrossFieldValidations:
    ...
    @pytest.mark.parametrize("monthly, tenure, total", [
        (100.0, 12, 1200.0),
        (100.0, 12, 1080.0),
        (65.5, 24, 1570.0),
    ])
    def test_total_vs_monthly_charges_valid(self, valid_input_data, monthly, tenure, total):
...
class TestToDictConversion:
    ...
    def test_to_dict_binary_conversions(self, valid_input_data, binary_field, input_value, expected_output_key, expected_value):
